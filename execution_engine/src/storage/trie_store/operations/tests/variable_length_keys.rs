...
impl ToBytes for VariableLengthKey {
    ...
    fn serialized_length(&self) -> usize {
        1 + self.key_bytes.serialized_length()
    }
}

impl FromBytes for VariableLengthKey {
    fn from_bytes(bytes: &[u8]) -> Result<(Self, &[u8]), bytesrepr::Error> {
        let (length, mut remainder) = u8::from_bytes(bytes)?;
        ...
        Ok((VariableLengthKey { key_bytes }, remainder))
    }
}
...
fn prefixed_binary_variable_length_key_arb() -> impl Strategy<Value = VariableLengthKey> {
    (0usize..7)
        .prop_flat_map(|vec_length| vec(binary_arb(), vec_length))
        .prop_map(|key_bytes| VariableLengthKey { key_bytes })
}

fn variable_length_key_test_inputs_arb(
    size: usize,
) -> impl Strategy<Value = BTreeMap<VariableLengthKey, u64>> {
    vec(
    ...
    .prop_map(|kv_pairs| {
        kv_pairs
            .into_iter()
            .collect::<BTreeMap<VariableLengthKey, u64>>()
    })
}

fn test_insert_variable_length_key_pairs(
    pairs_to_insert: BTreeMap<VariableLengthKey, u64>,
) -> anyhow::Result<()> {
    ...
    let context = InMemoryTestContext::new(&[empty_trie])?;
    {
        for (key, value) in pairs_to_insert.iter() {
            ...
            if let WriteResult::Written(new_root) =
                write::<VariableLengthKey, u64, _, InMemoryTrieStore, InMemoryError>(
    ...
    }
    {
        ...
        let mut keys_found_in_input = pairs_to_insert
            .keys()
            .cloned()
            .collect::<Vec<VariableLengthKey>>();
        keys_found_in_input.sort();
        let mut keys_found_in_trie =
            keys_with_prefix::<VariableLengthKey, u64, _, InMemoryTrieStore>(
                correlation_id.clone(),
                &txn,
                &context.store,
                &root,
